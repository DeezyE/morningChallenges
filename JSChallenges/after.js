...
// Your code here!
const after = (time, runThis) => {
    const runCount = (time, runThis) => {
        if( typeof runCount.counter === 'undefined' ) {
            runCount.counter = 0;
        }
        runCount.counter++;
        // console.log('Just incremented', runCount.counter)
        if (runCount.counter === time) {
            runThis
        } else {
        }
    }
